  ...
  private analyzeCookies(cookiesData: any): void {
    ...
    if (thirdPartyCookies > 0) {
      this.addIssue({
        ...
        recommendation: 'Implémenter un système de consentement conforme RGPD pour tous les cookies tiers',
  ...
  private analyzeServerLocation(locationData: any): void {
    ...
    if (nonEUCountries.includes(locationData.countryCode)) {
      this.addIssue({
        ...
        article: 'Article 44-49 RGPD - Transferts internationaux',
  ...
  private analyzeLegalPages(legalData: any): void {
    ...
    if (missingPages.length > 0) {
      this.addIssue({
        ...
        article: 'Article 13-14 RGPD - Information des personnes',
  ...
  private generateRecommendations() {
    ...
    if (criticalIssues > 0) {
      recommendations.push({
        ...
        description: `${criticalIssues} problème(s) critique(s) nécessitent une action immédiate pour assurer la conformité RGPD`,
        actions: [
